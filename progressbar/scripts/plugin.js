(function ( $ ) {
        ...
        animate : function(value, time) {
            ...
            var theInterval = setInterval(function() {
                if(global_settings.percent < value) {
                    ...
                    var timeLapse = ((timeEnd-timeStart)/1000).toString();
                    timeLapse = timeLapse.split(".");
                    timeLapse = timeLapse[0];

                    $('#progressTitle').html("Completed!");
                    $('#progressTitle').css("color","#1ABC9C");

                    $('#timeLapse').html("Time taken: "+timeLapse+" seconds");
                    $('#timeLapse').css("color","#1ABC9C");
    ...
    function insertText(scope) {
        ...
        $(".progress-percentage").css('margin-top','30px');
        $(".progress-percentage").css('color','#1ABC9C');
    }
    // create canvas
